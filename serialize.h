...
nlohmann::json toJson(const FlatMap<KeyType, ValueType, Size>& mp) {
    nlohmann::json j;
...
nlohmann::json toJson(const std::array<T, N>& arr) {
    nlohmann::json j;
    j["values"] = nlohmann::json::array();
...
template <typename T, size_t N>
nlohmann::json toJson(const ResizableArray<T, N>& arr) {
    nlohmann::json j;
    j["values"] = nlohmann::json::array();
    for (const auto& v : arr) {
        j["values"].push_back(toJson(v));
